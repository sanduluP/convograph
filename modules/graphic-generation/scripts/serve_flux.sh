...
# Weights live OUTSIDE the repo: sync_to_cluster.sh rsyncs with --delete, so a
# cache inside REPO_ROOT would be wiped and re-downloaded on every sync. And on
# unicorn $HOME is ~95% full, so the HuggingFace default (~/.cache) is wrong too.
